...
export function prepararRespuesta(id,seleccion,detalle=''){if(!OTROS[id]||seleccion!==OTROS[id])return seleccion??'';const texto=String(detalle).trim();if(texto.length<2||texto.length>400)throw Error('Escribe tu opción en el campo Otro (entre 2 y 400 caracteres).');return {opcion:seleccion,texto};}
